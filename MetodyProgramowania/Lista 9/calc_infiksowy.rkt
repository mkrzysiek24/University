#lang plait


(define-type Op
  (op-add) (op-mul) (op-sub) (op-div))

(define-type Exp
  (exp-number [n : Number])
  (exp-op [op : Op] [e1 : Exp] [e2 : Exp]))

(define-type (ParseResult 'a)
  (parse-err)
  (parse-ok [v : 'a] [rest : (Listof S-Exp)]))

(define (parse-op0 ss)
  (type-case (Listof S-Exp) ss
    [empty (parse-err)]
    [(cons op rest)
     (cond
       [(s-exp-match? `+ op) (parse-ok (op-add) rest)]
       [(s-exp-match? `- op) (parse-ok (op-sub) rest)]
       [else (parse-err)])]))

(define (parse-exp0 ss)
  (type-case (ParseResult Exp) (parse-exp1 ss)
    [(parse-err) (parse-err)]
    [(parse-ok e1 rest)
     (parse-exp0-helper e1 rest)]))

(define (parse-exp0-helper e1 ss)
  (begin (display e1)
         (display "\n")
         (display ss)
         (display "\n")
         (display "\n")
  (type-case (ParseResult Op) (parse-op0 ss)
    [(parse-err)         (parse-ok e1 ss)]
    [(parse-ok op rest2)
     (type-case (ParseResult Exp) (parse-exp1 rest2)
       [(parse-err) (parse-err)]
       [(parse-ok e2 rest3) (parse-exp0-helper (exp-op op e1 e2) rest3)])])))

(define (parse-op1 ss)
  (type-case (Listof S-Exp) ss
    [empty (parse-err)]
    [(cons op rest)
     (cond
       [(s-exp-match? `* op) (parse-ok (op-mul) rest)]
       [(s-exp-match? `/ op) (parse-ok (op-div) rest)]
       [else (parse-err)])]))

(define (parse-exp1 ss)
  (type-case (ParseResult Exp) (parse-exp2 ss)
    [(parse-err) (parse-err)]
    [(parse-ok e1 rest)
     (parse-exp1-helper e1 rest)]))

(define (parse-exp1-helper e1 ss)
  (type-case (ParseResult Op) (parse-op1 ss)
    [(parse-err)         (parse-ok e1 ss)]
    [(parse-ok op rest2)
     (type-case (ParseResult Exp) (parse-exp1 rest2)
       [(parse-err) (parse-err)]
       [(parse-ok e2 rest3) (parse-exp1-helper (exp-op op e1 e2) rest3)])]))

(define (parse-exp2 ss)
  (type-case (Listof S-Exp) ss
    [empty (parse-err)]
    [(cons s rest)
     (cond
       [(s-exp-number? s) (parse-ok (exp-number (s-exp->number s)) rest)]
       [(s-exp-list? s)
        (type-case (ParseResult Exp) (parse-exp0 (s-exp->list s))
          [(parse-err) (parse-err)]
          [(parse-ok e rest2)
           (if (empty? rest2)
               (parse-ok e rest)
               (parse-err))])])]))

(define (parse-exp s)
  (type-case (ParseResult Exp) (parse-exp0 (list s))
    [(parse-err) (error 'parse-exp "Syntax error")]
    [(parse-ok e rest)
     (if (empty? rest)
         e
         (error 'parse-exp "Syntax error"))]))

(parse-exp `(4 + 2 + 3 * 6))