...
(define (decompile [c : Code])
  (local [(define (decompile-helper code stack)
            (type-case Code code
              [empty (first stack)]
              [(cons i r) (type-case Instr i
                            [(pushI n) (decompile-helper
                                        r
                                        (cons (number->s-exp (pushI-n i)) stack))]
                            [(opI n) (decompile-helper
                                      r
                                      (cons (list->s-exp (list (op-check i) (second stack) (first stack)))
                                            (rest (rest stack))))])]))]
    (decompile-helper c (list))))



(compile (parse `{+ {* 2 {+ 4 5}} {+ 2 3}}))
(decompile (compile (parse `{+ {* 2 {+ 4 5}} {+ 2 3}})))