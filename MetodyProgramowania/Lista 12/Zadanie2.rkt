...
(define-type Value
  ...
  (funAV [e : ExpA] [env : (EnvA 'a)])) ; konstruktor reprezentujący funkcję w postaci ExpA
...
(define (value->string [v : Value]) : String
  (type-case Value v
    ...
    [(funAV e env)   "#<procedure>"])) ; W wypisywaniu uwzględniamy nowo powstały konstruktor
...
(define-type ExpA
  ...
  (lambdaA [e : ExpA])          ; Wprowadzamy konstruktor dla lambd
  (appA [f : ExpA] [a : ExpA])) ; Wprowadzamy konstruktor dla aplikacji
...
(define (evalA [a : ExpA] [env : (EnvA Value)]) : Value
  (type-case ExpA a
    ...
    [(lambdaA e)                               
     (funAV e env)]                            ; wrzucamy lambde do funAV
    [(appA e1 e2)                              
     (applyA (evalA e1 env) (evalA e2 env))])) ; APPLY Z WYLICZONYMI ARGUEMNTAMI
                                               

(define (applyA [v1 : Value] [v2 : Value]) : Value ; Funckja applyA TO PRAWIE APPLY
  (type-case Value v1                              
    [(funAV e env)
     (evalA e (extend-envA env v2))]
    [else (error 'apply "not a function")]))
...
(define (translate [e : Exp] [env : (EnvA Symbol)]) : ExpA
  (type-case Exp e
    ...
    [(lamE x e)                                      
     (lambdaA (translate e (extend-envA env x)))]    ; PRAWIE JAK LET
    [(appE e1 e2)                                   
     (appA (translate e1 env) (translate e2 env))])) ;WYWOLUJEMY NA ARG
...
;Taka reprezentacja z indeksami dobrze współgra z domknięciami funkcji,
;ponieważ indeksy odzwierciedlają odległość między miejscem definiowania
;zmiennej a miejscem jej użycia. Dzięki temu, nawet jeśli funkcja zostanie
;przekazana gdzieś indziej i będzie wywoływana poza swoim pierwotnym zasięgiem,
;jej domknięcie będzie nadal zawierać informacje o miejscu związania zmiennych.
