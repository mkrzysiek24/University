...
(define (apply [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    ...
    [(recV name x exp env)
     (eval exp (extend-env (extend-env env name v1) x v2))] ;uzywamy name, wiec musimy ddodac je do srodowiska
    [(primopV f)
     (f v2)]     
...
; rozbudowująć język pod funkcje rec, korzystamy z faktu że zostanie ona opakowana przez parser
; w konstrukcje appE, przez co możemy wywołać ją we wlasnym ciele, ponieważ jestesmy w stanie 
; odpowiednio rozszerzyc srodowisko.
; A letrec nie jest opakowywany w appE, stąd nie będziemy mogli się odwołać do funkcji w jej ciele