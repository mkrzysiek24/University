  ...
  (cond
    ...
    [(s-exp-match? `{let SYMBOL ANY ANY} s)
     ...
     [(s-exp-match? `{letrec SYMBOL ANY ANY} s)
      (let* ([name-func (s-exp->symbol (second (s-exp->list s)))]
        [arg (parse (fourth (s-exp->list s)))]
        [fix (parse `(lambda (f)
                       (let w (lambda (g) (f (lambda (z) ((g g) z))))
                         (w w))))])
        (letE name-func
          (appE fix (lamE name-func (parse (third (s-exp->list s)))))
         arg))]
         ...
         (λ (v2)
           (type-case Value v2
             ...
             [else
              (error 'eval "type error")])))]
       [else
        (error 'eval "type error")])))) ;<- tu error
