...
(define (wire-on-change! wire action) ;done
  ...
  (let [(new_val (if (equal? (action-function action) logical-not)
                     ((action-function action) (wire-val (action-input1 action)))
                     ((action-function action) (wire-val (action-input1 action)) (wire-val (action-input2 action)))))]
    (wire-set! (action-output action) new_val)))
