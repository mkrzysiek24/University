...
(define fac
  (stream-cons 1 (map2 * fac (stream-cdr nats2))))

(define (partial-sums stream n)
  (letrec ([new-stream (stream-cons (stream-car stream)
                                    (map2 + (stream-cdr stream) new-stream))])
    (stream-ref new-stream n)))
